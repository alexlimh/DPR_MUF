...
def calculate_matches(
    ...
    if workers_num > 1:
        ...
        scores = list(map(get_score_partial, questions_answers_docs))
    if not silent:
    ...
    if get_best_index:
        batch_hit_indices = []
        block_size = len(scores) // batch_size
        for i in range(batch_size):
            hit_indices = []
            for question_hits in scores[i*block_size:(i+1)*block_size]:
                hit_index = [j for j, x in enumerate(question_hits) if x]
                hit_index = sum(hit_index) if len(hit_index) > 0 else -1
                hit_indices.append(hit_index)
            if np.mean(hit_indices) == -1.:
                batch_hit_indices.append(-1)
            else: 
                batch_hit_indices.append(np.argmin(hit_indices)+i*block_size)
        return batch_hit_indices, QAMatchStats(top_k_hits, scores)
