...
class BiEncoderTrainer(object):
    ...
    def __init__(self, args):
        ...
        if saved_state:
            self._load_saved_state(saved_state, args)
    ...
    def _train_epoch(
        ...
        for i, samples_batch in enumerate(
            ...
            if (i + 1) % rolling_loss_step == 0:
                ...
                logger.info(
                    "Avg. loss per last %d batches: %f (time: %f s/iter)",
                    rolling_loss_step,
    ...
    def _load_saved_state(self, saved_state: CheckpointState, args):
        ...
        pretrained_dict = saved_state.model_dict
        
        if args.load_ctx_encoder:
            pretrained_dict = {k:v for k,v in pretrained_dict.items() if 'ctx' in k}
...
def main():
    ...
    parser.add_argument('--load_ctx_encoder', action='store_true')
