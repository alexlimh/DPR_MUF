...
class ShardedDataIterator(object):
    ...
    def __init__(
        ...
        self.sample_orders = [np.arange(samples_per_shard, dtype=np.int32) for i in range(ensemble_size)]
    ...
    def iterate_data(self, epoch: int = 0) -> Iterator[List]:
        if self.shuffle:
            ...
            for sample_order in self.sample_orders:
        ...
        for i in range(
            ...
            if self.ensemble_size == 1:
                ...
                items = []
                for sample_order in self.sample_orders:
                    index_snippet = sample_order[i : i + self.batch_size]
                    for index in index_snippet:
                        items.append(shard_samples[index])
