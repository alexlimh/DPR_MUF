...
class DenseReconIndexer(DenseIndexer):
    def __init__(self, vector_sz, buffer_size: int = 50000):
        super(DenseReconIndexer, self).__init__(buffer_size=buffer_size)
        nlist = 100
        quantizer = faiss.IndexFlatIP(vector_sz)
        self.vector_sz = vector_sz
        self.index = faiss.IndexIVFFlat(quantizer, vector_sz, nlist, faiss.METRIC_INNER_PRODUCT)
        self.index.set_direct_map_type(faiss.DirectMap.Hashtable)

    def _index_batch(self, data: List[Tuple[object, np.array]]):
        db_ids = np.array([t[0] for t in data], dtype=np.int64)
        vectors = [np.reshape(t[1], (1, -1)) for t in data]
    ...
    def reconstruct(self, ctx_id):
        return self.index.reconstruct(ctx_id)
